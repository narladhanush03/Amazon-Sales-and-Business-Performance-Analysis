...
--VIEW
-----------------------------
CREATE OR REPLACE VIEW amazon_discount_analysis AS
...
discount_quartiles AS (
    SELECT
        PERCENTILE_CONT(0.25)
            WITHIN GROUP (ORDER BY discount) AS q1,
        PERCENTILE_CONT(0.50)
            WITHIN GROUP (ORDER BY discount) AS median,
        PERCENTILE_CONT(0.75)
            WITHIN GROUP (ORDER BY discount) AS q3
    FROM distinct_delivered_discounts)
...
SELECT
    ...
    profit,
    CASE
        WHEN delivery_status <> 'Delivered' THEN 'Not Applicable'
        WHEN discount = 0 THEN 'No Discount'
        WHEN discount <= q1 THEN 'Low Discount'
        WHEN discount <= median THEN 'Medium Discount'
        WHEN discount <= q3 THEN 'High Discount'
        ELSE 'Very High Discount'
    END AS discount_band
FROM amazon_sales_clean
CROSS JOIN discount_quartiles;
